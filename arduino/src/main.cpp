...
int contador_sincronizacion_limite = 3600; // Sincronizar cada 3600 segundos = 1 hora
bool sincronizar_rtc = false;
...
void loop()
{
  ...
  if (contador_sincronizacion >= contador_sincronizacion_limite)
  {
    int segundo = rtc.now().second();

    // Sincroniza sólo si no está justo al inicio de un nuevo minuto (para evitar problemas)
    if (segundo > 7 && segundo < 45)
    {
      sincronizar_rtc = true;
    }
  }
  ...
  if (sincronizar_rtc)
  {
    ...
    activarEthernet();      // Usá Ethernet
    sincronizarRTCconNTP(); // Sincronizá con NTP
    activarSD();            // Volvé a SD
    sincronizar_rtc = false;
...
{
  Udp.begin(8888);
  Serial.println("Solicitando hora NTP...");
  ...
  if (Udp.parsePacket())
  {
    ...
    Serial.println(obtener_hora(ntpTime));
    Serial.println(obtener_fecha(ntpTime));
  }
  else
  {
